...
class TestSetupStrategy(Strategy):
    ...
    def simple_run(
        ...
        for [character_id, moves] in possible_moves.items():
            ...
            is_traceur = game_state.characters[character_id].class_type == CharacterClassType.TRACEUR
            is_demo = game_state.characters[character_id].class_type == CharacterClassType.DEMOLITIONIST
            ...
            if is_demo:
                ...
                wall_2 = "86,92"
                goal = Position(50, 83)
                if (wall_1 in obstacle_set) or wall_2 in obstacle_set: #bfs to 85, 91
                    goal = Position(85, 91)
                    
                else:
                    goal = Position(99, 99)
                new_pos = self.simple_bfs(goal, pos, game_state, 3)
                new_action = MoveAction(character_id, new_pos)
                choices.append(new_action)
